...
class DAN_Model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.layer5(x)
        x = self.layer6(x)
        upper_output = self.layer7(x)
        upper_output1 = self.layer8(upper_output)
        output = self.layer9(upper_output1)
        return output, upper_output, upper_output1

class DAN(nn.Module):
    def __init__(self):
        super().__init__()
        self.ddc_model = DAN_Model()

    def forward(self, source_inputs, target_inputs):
        s_output, s_upper_output, s_upper_output1 = self.ddc_model(source_inputs)
        t_output, t_upper_output, t_upper_output1 = self.ddc_model(target_inputs)
        return s_output, t_output, s_upper_output, t_upper_output, s_upper_output1, t_upper_output1


def train(source_data, target_data, source_label, target_label, source_vec_label, epochs=100, batch_size=50):
    # TODO: train
    ddc = DAN()
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    # 使用MMD损失函数进行训练
    lmmd_loss = MMDLoss(kernel_type='gaussian', kernel_mul=5.0, kernel_num=20, fix_sigma=None, degree=5)
    viz = Visdom(env="main")
    ...
    for epoch in range(epochs):
        for i in range(source_data.shape[0]//batch_size):
            ...
            # 训练
            source_outputs, target_outputs, s_x, t_x, s_x1, t_x1 = ddc(source_data_temp, target_data_temp)
            # 计算损失
            # loss = lmmd_loss(s_x, t_x) + loss_func(source_outputs, torch.from_numpy(source_vec_label_temp).to(device)) + \
            # lmmd_loss(s_x1, t_x1) + lmmd_loss(source_outputs, target_outputs)
            loss = lmmd_loss(s_x, t_x) + loss_func(source_outputs, torch.from_numpy(source_vec_label_temp).to(device)) + \
                   lmmd_loss(s_x1, t_x1)
            loss.backward()
            optimizer.step()
        source_outputs, target_outputs, _, _, _, _ = ddc(source_data, target_data)
        # 计算准确率，源域
